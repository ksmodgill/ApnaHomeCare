...
export default function LocationModal({
  ...
  const { siteSettings, homepage } = useSiteContent();
  const services = homepage.servicesSection?.services || [];
            ...
            <div className="overflow-y-auto px-5 py-5 md:px-6 md:py-6">
              ...
              <ul className="mt-5 space-y-2.5">
                {HIGHLIGHTS.map((item) => (
                  <li
                    key={item}
                    className="flex items-center gap-2 text-sm text-secondary md:text-base"
                  >
